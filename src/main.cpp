...
uint256 hashGenesisBlock("0x88331c37110f214c79373e944b1b6a2fcc9b102dc898c7af5998fccfd7a1458e");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // InstaMineNuggets: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "InstaMineNuggets Signed Message:\n";
...
{
    // InstaMineNuggets: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    if(nHeight == 1){
        nSubsidy = nSubsidy * 21649400 * 3  / 100 / 30;
     }
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // InstaMineNuggets: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // InstaMineNuggets: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // InstaMineNuggets: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x88331c37110f214c79373e944b1b6a2fcc9b102dc898c7af5998fccfd7a1458e");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1422565091;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084963468;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("InstaMineNuggetsMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("InstaMineNuggetsMiner : ProcessBlock, block not accepted");
    }
...
void static LitecoinMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running InstaMineNuggets with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("InstaMineNuggetsMiner terminated\n");
        throw;
    }
}
